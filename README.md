...
- **Library** — `index` the bundles, `search` across every scribe, `prune` to remove videos (dry run unless `--yes`); `--frames-only` reclaims the disk but keeps the video searchable and rebuildable from its URL
