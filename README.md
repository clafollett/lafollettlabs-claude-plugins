...
| [watchwith](./plugins/watchwith/) | Watch a video and write down what was on screen — specs, reviews, summaries, walkthroughs | [Skill](./plugins/watchwith/skills/watchwith/SKILL.md) |
...
- **Build** — samples at 1 fps into `~/.watchwith/bundles`, drops visually-identical frames, names the survivors by timestamp
...
- **Library** — `index` the bundles, `search` across every scribe, `prune` to remove videos (dry run unless `--yes`)
- Name a video by its id, its URL, or any words from its title or channel — ambiguity is listed, never guessed
...
```
/plugins marketplace add https://github.com/lafollett-labs/lafollett-labs-claude-plugins
```
