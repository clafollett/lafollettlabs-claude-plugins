...
- **Built-in PE sub-agents** — `pe-backend` (Go/SQL/Lambda), `pe-frontend` (Vue/Nuxt/TS), `pe-devops` (CDK/CDKTF/IaC), each with `claude-opus-4-7` and a self-contained three-pass protocol
...
Three stack-specific PE agents ship with the plugin. Each runs the full three-pass protocol (Architecture → Quality+Tests → Security) within its domain and returns findings as YAML for the parent skill to consolidate.

| Agent | Domain | Test commands |
| ----- | ------ | ------------- |
| `code-reviewer:pe-backend` | Go / PostgreSQL / AWS Lambda | `go vet`, `go test -race`, `staticcheck` |
| `code-reviewer:pe-frontend` | Vue 3 / Nuxt 3 / TS / Tailwind / Storybook | `npm run typecheck`, `npm test`, `npm run build-storybook` |
| `code-reviewer:pe-devops` | AWS CDK / Cloudflare CDKTF / Terraform / Docker / GH Actions | `npm test`, `npx cdk synth`, `actionlint` |
